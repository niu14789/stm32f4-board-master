...
函数名：将uni2oem表IAP到spiflash 5,6,7,8,9,10扇区
...
void Updataoem(u8 num)
{
  u32 i=0;
  u16 addr=0;
  OKme_String(3,100,"更新中！！！！",Black,0xc618);
  sect_clr(5);		//327680
  sect_clr(6);
  sect_clr(7);	 //前三个扇区存放uni2oem
  sect_clr(8);		//524288
  sect_clr(9);  
  sect_clr(10);	 //后三个扇区存放oem2uni
  for(i=0;i<sizeof(uni2oem);i++)
  {
    oembuf[addr]=uni2oem[i]>>8;
  	addr++;
	oembuf[addr]=uni2oem[i];
	addr++;
  	if(addr==256)
	{
	 addr=0;
	 M25_W_BLOCK(1280+i/128,oembuf,256);
	}
  }
  addr=0;
  for(i=0;i<sizeof(oem2uni);i++)
  {
    oembuf[addr]=oem2uni[i]>>8;
  	addr++;
	oembuf[addr]=oem2uni[i];
	addr++;
  	if(addr==256)
	{
	 addr=0;
	 M25_W_BLOCK(2048+i/128,oembuf,256);
	}
  }
  OKme_String(3,100,"更新完成！！！",Black,0xc618);	 
}
#endif
/*------------------------------------------
mode:0,uni2oem
mode:1,oem2uni
------------------------------------------*/
u16 oem22uni(u8 mode,u32 offer)
{
  u8 buf[2];
  switch(mode)
  {
   	case 0:Readoem(327680+offer*2,buf);break;
  	case 1:Readoem(524288+offer*2,buf);break;
  }
...
)
{
	...
	if (src < 0x80) {	/* ASCII */
		...
		for (n = 16; n; n--) {
			i = li + (hi - li) / 2;
			if (src == oem22uni(oemflag,i * 2)) break;
			if (src > oem22uni(oemflag,i * 2))
				li = i;
			else
				hi = i;
		}
		c = n ? oem22uni(oemflag,i * 2+1) : 0;
	}
