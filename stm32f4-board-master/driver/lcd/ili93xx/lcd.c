...
//x1,y1:起点坐标
//x2,y2:终点坐标  
void LCD_DrawLine_ili(u16 x1, u16 y1, u16 x2, u16 y2,unsigned short color)
{
	u16 t; 
...
// {
// 	LCD_DrawLine_ili(x1,y1,x2,y1);
// 	LCD_DrawLine_ili(x1,y1,x1,y2);
// 	LCD_DrawLine_ili(x1,y2,x2,y2);
// 	LCD_DrawLine_ili(x2,y1,x2,y2);
// }
//在指定位置画一个指定大小的圆
//(x,y):中心点
//r    :半径
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString_ili(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p)
{         
	u8 x0=x;
	width+=x;
	height+=y;
...
void lcd_showlongstring_ili(void)
{
 	 u16 x=0;
	 u16 i,j;
	 for(i=0;i<26;i++)
