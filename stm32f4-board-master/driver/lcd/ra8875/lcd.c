...
{ 	
	...
	//FSMC读时序控制寄存器
	FSMC_ReadWriteTimingStructure.FSMC_AddressSetupTime = 4; //地址建立时间 16个HCLK =16X(1/168M)=96ns
	FSMC_ReadWriteTimingStructure.FSMC_AddressHoldTime = 0x00;  //地址保持时间模式A未用到 
	FSMC_ReadWriteTimingStructure.FSMC_DataSetupTime = 6;    //数据保存时间为25个HCLK 25x(1/168M)=150ns
	...
	//FSMC写时序控制寄存器
	FSMC_WriteTimingStructure.FSMC_AddressSetupTime = 4;  //地址建立时间为8个HCLK 8x(1/168M)=48ns
